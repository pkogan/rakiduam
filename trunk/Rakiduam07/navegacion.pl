...
:- module(navegacion,[ir_a_posicion07/5,gira/5,resolver/4,patear/6,tiene_pelota/1,llevar_pelota_to_arco/3,ir_a_posicion_y_apuntar/6,calcular_angulo/4,llevar_pelota_a_posicion/5], [assertions]).
:- use_module(ambiente).
:- use_module(logger).
...
llevar_pelota_a_posicion(robot(E,J,pos(Xr,Yr,_,Rr)),Xd,Yd,Vi,Vd):-
	...
	ir_a_posicion07(robot(E,J,pos(Xr,Yr,_,Rr)),Xd,Yd,Vi,Vd).
...
ir_a_posicion_y_apuntar(robot('propio',_Number,pos(Xr,Yr,_Z,_Rr)), X,Y,_Angulo,Vi,Vd) :-
	...
	Distancia is sqrt(Dx**2 + Dy**2), % pitáagoras distancia al objetivo (X,Y)
	\+(distancia_cero(Distancia)),
	ir_a_posicion07(robot('propio',_Number,pos(Xr,Yr,_Z,_Rr)),X,Y,Vi,Vd).
...
ir_a_posicion07(robot('propio',_Number,pos(Xr,Yr,_Z,_Rr)), X,Y,0,0) :-
	Dx is X-Xr,
	Dy is Y-Yr,
	Distancia is sqrt(Dx**2 + Dy**2), % pitáagoras distancia al objetivo (X,Y)
	distancia_cero(Distancia).

ir_a_posicion07(robot('propio',_Number,pos(Xr,Yr,_Z,Rr)), X,Y,Vi,Vd) :-
	Dx is X-Xr,
	Dy is Y-Yr,
...
:- pred ir_a_posicion07(+Xr,+Yr,+Rr,+X,+Y,+Vc,-Vi,-Vd) :: int * int * int * int * int * int * int * int 
...
llevar_pelota_to_arco(Robot,Vi,Vd) :-
	ir_a_posicion07(Robot,20,41,Vi,Vd).
